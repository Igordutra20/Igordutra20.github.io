...
def senha_valida(senha):
    return bool(re.match(r'^(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$', senha))
...
def registrar():
    ...
    cursor.execute(
        "INSERT INTO usuarios (username, email, senha, confirmado, codigo_verificacao) VALUES (?, ?, ?, ?, ?)",
...
def confirmar():
    ...
    cursor.execute(
        "UPDATE usuarios SET confirmado = ?, codigo_verificacao = NULL WHERE email = ?",
        (True, email)
    )
    conn.commit()
